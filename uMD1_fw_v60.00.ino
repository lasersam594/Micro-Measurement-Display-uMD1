#define FirmwareVersion 60.00
#define LowCPUClockEnable 0
...
// V60.00 added 20 MHz clock speed for DIY low REF lasers (down to around 700 kHz with interpolation).  Reliability questionable.
...
//    digitalWrite(PIN_LED3, 1); // Turn on LED3 at start of axis capture and analysis
...
// Adjust CPU clock speed to optimize interpolation.
   ...
   if (ClkSkipCount > 0)
     ...
     {
      if (CPUClock == 20000000) // 20 MHz
         {
           if (REFFrequency > 1120000)
             {
               CPUClock = 30000000;
       ...
       else if (CPUClock == 30000000) // 30 MHz
         {
           if ((REFFrequency < 1075000) && (LowCPUClockEnable == 1))
             {
               CPUClock = 20000000;
               OSCConfig (OSC_POSC_PLL,OSC_PLL_MULT_20,OSC_PLL_POST_4,OSC_FRC_POST_2); // 20 Mhz
               SampleFrequencyX100 = 30517;
               SampleFrequency = 305;
               ClkSkipCount = 2;
             }
           else if (REFFrequency > 1490000)
             {
               CPUClock = 40000000;
               OSCConfig (OSC_POSC_PLL,OSC_PLL_MULT_20,OSC_PLL_POST_2,OSC_FRC_POST_2); // 40 Mhz
               SampleFrequencyX100 = 61035;
               SampleFrequency = 610;
               ClkSkipCount = 2;
             }
         }
       else if (CPUClock == 40000000) // 40 MHz
         {
           if (REFFrequency < 1430000)
             {
               CPUClock = 30000000;
               OSCConfig (OSC_POSC_PLL,OSC_PLL_MULT_15,OSC_PLL_POST_2,OSC_FRC_POST_2); // 30 Mhz
               SampleFrequencyX100 = 45776;
               SampleFrequency = 458;
               ClkSkipCount = 2;
             }
           else if (REFFrequency > 1780000)
             {
               CPUClock = 48000000;
               OSCConfig (OSC_POSC_PLL,OSC_PLL_MULT_24,OSC_PLL_POST_2,OSC_FRC_POST_2); // 48 Mhz
               SampleFrequencyX100 = 73242;
               SampleFrequency = 732;
               ClkSkipCount = 2;
             }
         }
       else if (CPUClock == 48000000) // 48 MHz
   {
           if (REFFrequency < 1720000)
             {
               CPUClock = 40000000;
               OSCConfig (OSC_POSC_PLL,OSC_PLL_MULT_20,OSC_PLL_POST_2,OSC_FRC_POST_2); // 40 Mhz
               SampleFrequencyX100 = 61035;
